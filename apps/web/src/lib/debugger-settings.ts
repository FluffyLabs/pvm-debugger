...
export type AutoContinuePolicy = "always_continue" | "continue_when_trace_matches" | "never";
...
export const STEPPING_MODES: { id: SteppingMode; label: string; hint: string }[] = [
  { id: "instruction", label: "Instruction", hint: "Single instruction per step, breakpoints enabled" },
  { id: "block", label: "Block", hint: "Step to next basic block boundary" },
  { id: "n_instructions", label: "N-Instructions", hint: "Step a custom number of instructions per step" },
];

export const AUTO_CONTINUE_OPTIONS: { id: AutoContinuePolicy; label: string; hint: string }[] = [
  { id: "always_continue", label: "Always", hint: "Automatically continue past all host calls using trace data" },
  {
  ...
  { id: "never", label: "Never (Manual)", hint: "Always pause on host calls for manual review" },
];
...
export function loadSettings(): DebuggerSettings {
  try {
    ...
    if (raw) {
      ...
      return {
        selectedPvmIds: validPvmIds(parsed.selectedPvmIds) ?? DEFAULT_SETTINGS.selectedPvmIds,
        steppingMode: validSteppingMode(parsed.steppingMode) ?? DEFAULT_SETTINGS.steppingMode,
        nInstructionsCount: validCount(parsed.nInstructionsCount) ?? DEFAULT_SETTINGS.nInstructionsCount,
        autoContinuePolicy:
          validAutoContinue(parsed.autoContinuePolicy) ?? DEFAULT_SETTINGS.autoContinuePolicy,
      };
    }
  } catch {
...
export function saveSettings(settings: DebuggerSettings): void {
  try {
    ...
    localStorage.setItem(PER_KEY_PREFIX + "selectedPvmIds", JSON.stringify(settings.selectedPvmIds));
    localStorage.setItem(PER_KEY_PREFIX + "steppingMode", settings.steppingMode);
    localStorage.setItem(PER_KEY_PREFIX + "nInstructionsCount", String(settings.nInstructionsCount));
    localStorage.setItem(PER_KEY_PREFIX + "autoContinuePolicy", settings.autoContinuePolicy);
  } catch {
...
function validSteppingMode(v: unknown): SteppingMode | null {
  ...
  return typeof v === "string" && valid.includes(v as SteppingMode) ? (v as SteppingMode) : null;
}
...
function validAutoContinue(v: unknown): AutoContinuePolicy | null {
  const valid: AutoContinuePolicy[] = ["always_continue", "continue_when_trace_matches", "never"];
