...
export function StructEditor({
  ...
  // Per-variant state
  const [constants, setConstants] = useState<ProtocolConstants>(
    DEFAULT_PROTOCOL_CONSTANTS,
  );
  ...
  const [refinementCtx, setRefinementCtx] = useState<RefinementContext>(
    DEFAULT_REFINEMENT_CONTEXT,
  );
  const [workItemSummary, setWorkItemSummary] = useState<WorkItemSummary>(
    DEFAULT_WORK_ITEM_SUMMARY,
  );
  const [allWorkItems, setAllWorkItems] = useState<WorkItemSummary[]>([]);
  const [transferOrOperand, setTransferOrOperand] =
    useState<TransferOrOperand>(DEFAULT_OPERAND);
  const [allTransfers, setAllTransfers] = useState<TransferOrOperand[]>([]);
  const [workPackage, setWorkPackage] =
    useState<WorkPackageData>(DEFAULT_WORK_PACKAGE);
  ...
  useEffect(() => {
    ...
    if (
      decodedBlobRef.current &&
      areBytesEqual(decodedBlobRef.current, initialBlob)
    )
      return;
  ...
  const encodedBlob = useMemo((): Uint8Array => {
    switch (kind) {
      ...
      case FetchKind.AuthorizationToken:
      case FetchKind.WorkItemPayload:
        return encodeVariantData({ kind, data: blobData } as {
          kind: FetchKind.AuthorizerTrace;
          data: Uint8Array;
        });
      case FetchKind.WorkPackage:
  ...
  const editor = (() => {
    switch (kind) {
      case FetchKind.Constants:
        return (
          <ProtocolConstantsEditor value={constants} onChange={setConstants} />
        );
      case FetchKind.Entropy:
        return (
          <div className="flex flex-col gap-1">
            <span className="text-[10px] text-muted-foreground">
              32-byte entropy hash
            </span>
            <BytesBlobEditor
              value={entropy}
              onChange={setEntropy}
      ...
      case FetchKind.WorkPackage:
        return (
          <WorkPackageEditor value={workPackage} onChange={setWorkPackage} />
        );
      case FetchKind.RefineContext:
        return (
          <RefinementContextEditor
            value={refinementCtx}
            onChange={setRefinementCtx}
          />
        );
      case FetchKind.AllWorkItems:
        return (
          <AllWorkItemsEditor value={allWorkItems} onChange={setAllWorkItems} />
        );
      case FetchKind.OneWorkItem:
        return (
          <WorkItemInfoEditor
            value={workItemSummary}
            onChange={setWorkItemSummary}
          />
        );
      case FetchKind.AllTransfersAndOperands:
        return (
          <AllTransfersEditor value={allTransfers} onChange={setAllTransfers} />
        );
      case FetchKind.OneTransferOrOperand:
        return (
          <TransferOrOperandEditor
            value={transferOrOperand}
            onChange={setTransferOrOperand}
          />
        );
      default:
        return (
  ...
  return (
    <div data-testid="struct-editor" className="flex flex-col gap-3">
      ...
      <div
        data-testid="struct-encoded-output"
