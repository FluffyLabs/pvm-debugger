...
function parseCommand(text: string, lineNum: number): ParsedCommand | ParseError {
  ...
  if (setregMatch) {
    ...
    if (regIndex < 0 || regIndex > 12) {
      return { line: lineNum, message: `Line ${lineNum}: Register index ${regIndex} out of range (0-12)` };
    }
    try {
      const value = BigInt(setregMatch[2].trim());
      return { kind: "setreg", regIndex, value };
    } catch {
      return { line: lineNum, message: `Line ${lineNum}: Invalid value "${setregMatch[2].trim()}"` };
    }
  }

  // memwrite 0x00001000 len=4 <- 0xaabbccdd
  const memMatch = trimmed.match(/^memwrite\s+(0x[\da-fA-F]+)\s+len=(\d+)\s*<-\s*(0x[\da-fA-F]*)$/i);
  if (memMatch) {
    ...
    if (hexStr.length !== len * 2) {
      return { line: lineNum, message: `Line ${lineNum}: len=${len} does not match hex data length ${hexStr.length / 2}` };
    }
  ...
  if (gasMatch) {
    try {
      ...
      return { line: lineNum, message: `Line ${lineNum}: Invalid gas value "${gasMatch[1].trim()}"` };
    }
  }

  return { line: lineNum, message: `Line ${lineNum}: Malformed command: "${trimmed}"` };
}

/** Parse all command lines, returning effects and any errors. */
export function parseAllCommands(text: string): { effects: HostCallEffects; errors: string[] } {
  ...
  const sortedRegs = new Map([...regWrites.entries()].sort(([a], [b]) => a - b));
...
function proposalToText(info: HostCallInfo): string {
  ...
  const sortedRegs = [...proposal.registerWrites.entries()].sort(([a], [b]) => a - b);
  for (const [idx, val] of sortedRegs) {
    lines.push(`setreg r${String(idx).padStart(2, "0")} <- 0x${val.toString(16)}`);
  }

  for (const mw of proposal.memoryWrites) {
    const hexData = Array.from(mw.data).map((b) => b.toString(16).padStart(2, "0")).join("");
    lines.push(`memwrite 0x${mw.address.toString(16).padStart(8, "0")} len=${mw.data.length} <- 0x${hexData}`);
  }
...
export function GenericHostCall({ info, onEffectsReady, traceVersion }: GenericHostCallProps) {
  ...
  return (
    <div data-testid="generic-host-call" className="flex flex-col gap-2 text-xs">
      ...
      />
      {errors.length > 0 && (
        <div data-testid="generic-errors" className="text-red-400 text-xs space-y-0.5">
          {errors.map((err, i) => (
            <div key={i}>{err}</div>
          ))}
        </div>
